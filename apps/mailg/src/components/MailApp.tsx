...
export default function MailApp() {
  ...
  return <div className={`mg-app theme-${theme}`}>
    ...
    {mailboxMenu && <div className="mg-account-popover"><span className="mg-account-user-avatar"><Icon name="person" size={32}/></span><strong>{connected ? "Banger account" : "mailG"}</strong><small>{connected ? "Signed in to Banger" : "Connect your Banger account"}</small>{connected ? <button onClick={() => void mailClient.signOut()}>Sign out</button> : <button onClick={() => mailClient.signIn()}>Connect Banger</button>}</div>}
