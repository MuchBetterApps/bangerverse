...
import { CustomizeDialog, OnboardingDialog } from "./OnboardingDialog";
...
export default function MailApp() {
  ...
  useEffect(() => {
    if (!mailboxMenu) return;
    const dismiss = (event: KeyboardEvent) => { if (event.key === "Escape") setMailboxMenu(false); };
  ...
  useEffect(() => {
    ...
    }).catch(error => { setToast(error.message); startDemo(); }).finally(() => setSessionReady(true));
  }, []);

  useEffect(() => {
    if (!sessionReady || !(connected ? inboxReady : demoEntered) || chooserOpen || openingMailbox) return;
    if (localStorage.getItem("mailg-customize-seen:v1")) return;
    const timer = setTimeout(() => { setCustomizeOpen(true); localStorage.setItem("mailg-customize-seen:v1", "true"); }, 1200);
    return () => clearTimeout(timer);
  }, [sessionReady, connected, inboxReady, demoEntered, chooserOpen, openingMailbox]);

  useEffect(() => {
    const onError = (event: Event) => setToast((event as CustomEvent<string>).detail);
    window.addEventListener("mailg-auth-error", onError);
    return () => window.removeEventListener("mailg-auth-error", onError);
  }, []);
  ...
  const load = useCallback(async (append = false, nextCursor?: string | null) => {
    ...
    try {
      ...
      if (!append && !query && page.mailboxes) {
        ...
        if (valid.length) {
          setSelectedMailboxIds(valid);
          const target = valid.includes(mailboxId) ? mailboxId : valid[0];
          if (target !== mailboxId) { setThreads([]); setLabels([]); setMailboxId(target); setOpeningMailbox(true); return; }
        }
      ...
      if (mailboxId) { setOpeningMailbox(false); setInboxReady(true); }
    } catch (error) { const message = error instanceof Error ? error.message : "Could not load mail"; setToast(message); setOpenError(message); }
  ...
  return <div className={`mg-app theme-${theme}`}>
    <header className="mg-topbar">
      ...
      <div className="mg-top-actions"><IconButton icon="help" label="Customize mailG" onClick={() => setCustomizeOpen(true)} /><IconButton icon="settings" label="Settings" onClick={() => setSettingsOpen(true)} /><IconButton icon="apps" label="Apps" onClick={() => setToast("More apps are coming soon.")} /><button className="mg-avatar" title={connected ? "Banger account" : "Connect Banger"} aria-label={connected ? "Banger account" : "Connect Banger"} aria-expanded={mailboxMenu} aria-controls="mailg-account-menu" onClick={() => setMailboxMenu(v => !v)}><Icon name="person" size={21}/></button></div>
    </header>
    ...
    {mailboxMenu && <><div className="mg-account-backdrop" onClick={() => setMailboxMenu(false)}/><section id="mailg-account-menu" className="mg-account-popover" role="dialog" aria-label="Banger account"><div className="mg-account-header"><IconButton icon="close" label="Close account menu" onClick={() => setMailboxMenu(false)}/></div><div className="mg-account-group"><div className="mg-account-profile"><span className="mg-account-user-avatar"><Icon name="person" size={32}/></span><div className="mg-account-identity"><strong>{connected ? "Banger account" : "mailG"}</strong><small>{connected ? "Signed in to Banger" : "Connect your Banger account"}</small></div></div><label className="mg-account-mailbox"><span>Selected mailbox</span><div className="mg-account-mailbox-control"><ProductAvatar product={activeMailbox && productForMailbox(activeMailbox)} fallback={activeMailbox?.address || "Mailbox"}/><select aria-label="Selected mailbox" value={mailboxId} onChange={event => { setMailboxId(event.target.value); openView("inbox"); }}>{mailboxes.map(mailbox => <option key={mailbox.id} value={mailbox.id}>{mailbox.address}</option>)}</select></div></label><button className="mg-account-action" onClick={() => connected ? void mailClient.signOut() : mailClient.signIn()}><Icon name={connected ? "logout" : "login"} size={24}/><span>{connected ? "Sign out" : "Connect Banger"}</span></button></div></section></>}
    {settingsOpen && <div className="mg-overlay" onClick={() => setSettingsOpen(false)}><aside className="mg-settings" onClick={event => event.stopPropagation()}><div className="mg-settings-header"><h2>Quick settings</h2><IconButton icon="close" label="Close settings" onClick={() => setSettingsOpen(false)}/></div><h3>Theme</h3><div className="mg-theme-grid">{(["light", "dark", "landscape"] as Theme[]).map(option => <button key={option} className={theme === option ? "selected" : ""} onClick={() => setTheme(option)}><span className={`mg-theme-preview preview-${option}`}><i/><i/><i/></span><span>{option === "landscape" ? "Landscape" : option === "dark" ? "Dark" : "Default"}</span></button>)}</div><h3>Account</h3><p className="mg-settings-copy">{connected ? "Connected to your Banger account" : "Explore the demo or connect your Banger workspace to use live mail."}</p><button className="mg-outline-button" onClick={() => connected ? void mailClient.signOut() : mailClient.signIn()}>{connected ? "Disconnect" : "Connect Banger"}</button></aside></div>}
    ...
    {chooserOpen && <OnboardingDialog titleId="mailg-chooser-title" className="mg-modal mg-mailbox-chooser" onDismiss={selectedMailboxIds.length ? () => setChooserOpen(false) : undefined}><h2 id="mailg-chooser-title">Which mailboxes do you want here?</h2><p>Choose the mailboxes to keep in your sidebar. You can change this later in Manage.</p><div className="mg-chooser-list">{!mailboxes.length && <p>No mailboxes are available in this workspace yet.</p>}{mailboxes.map(mailbox => <label key={mailbox.id}><input type="checkbox" checked={chooserIds.includes(mailbox.id)} onChange={event => setChooserIds(ids => event.target.checked ? [...ids, mailbox.id] : ids.filter(id => id !== mailbox.id))}/><ProductAvatar product={productForMailbox(mailbox)} fallback={mailbox.display_name || mailbox.address} /><span><strong>{mailbox.display_name || mailbox.address}</strong><small>{mailbox.address}</small></span></label>)}</div><div className="mg-modal-actions"><span/>{!mailboxes.length && connected && <button onClick={() => void workspaceClient.signOut()}>Sign out</button>}{selectedMailboxIds.length > 0 && <button onClick={() => setChooserOpen(false)}>Cancel</button>}<button className="mg-primary-button" disabled={!chooserIds.length} onClick={() => { if (!chooserIds.length) return; setSelectedMailboxIds(chooserIds); localStorage.setItem(`mailg-mailboxes:${workspaceId || "demo"}`, JSON.stringify(chooserIds)); const target = chooserIds.includes(mailboxId) ? mailboxId : chooserIds[0];
        if (mode === "live") { ++loadSequence.current; setThreads([]); setLabels([]); setDrafts([]); setFilters([]); setInboxReady(false); setOpeningMailbox(true); }
        openView("inbox"); setMailboxId(target); setChooserOpen(false);
        if (mode === "live" && target === mailboxId && view === "inbox" && !query && !labelId) void load();
      }}>Open selected mailboxes</button></div></OnboardingDialog>}
    ...
    {sessionReady && connected && !chooserOpen && (!inboxReady || openingMailbox) && <OnboardingDialog titleId="mailg-opening-title"><h2 id="mailg-opening-title">{openError ? "Couldn’t open your inbox" : "Opening your inbox…"}</h2><p role="status">{openError || "Getting your mailboxes ready."}</p>{openError && <button className="mg-primary-button" onClick={() => void load()}>Try again</button>}</OnboardingDialog>}
    {customizeOpen && <CustomizeDialog onDismiss={() => setCustomizeOpen(false)}/>}
