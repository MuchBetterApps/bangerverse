...
type Mailbox = { id: string; address: string; display_name?: string };
...
export default function MailApp() {
  ...
  return <div className={`mg-app theme-${theme}`}>
    ...
    <div className="mg-body">
      <aside className="mg-app-rail" aria-label="Mailbox switcher">{mailboxes.filter(mailbox => selectedMailboxIds.includes(mailbox.id)).map(mailbox => <button key={mailbox.id} className={mailbox.id === mailboxId ? "active" : ""} title={mailbox.address} onClick={() => { setMailboxId(mailbox.id); openView("inbox"); }}><span className="mg-rail-letter">{(mailbox.display_name || mailbox.address)[0].toUpperCase()}</span><small>{mailbox.display_name || mailbox.address.split("@")[0]}</small></button>)}<button title="Choose mailboxes" onClick={() => { setChooserIds(selectedMailboxIds); setChooserOpen(true); }}><span><Icon name="plus" size={20}/></span><small>Manage</small></button></aside>
      ...
      <aside className="mg-utility-rail"><button title="Calendar" onClick={() => setToast("Calendar is not connected.")}><Icon name="calendar" size={20}/></button><button title="Notes" onClick={() => setToast("Notes are not connected.")}><Icon name="picture" size={20}/></button><button title="Tasks" onClick={() => setToast("Tasks are not connected.")}><Icon name="tasks" size={20}/></button><button title="Contacts" onClick={() => setToast("Contacts are not connected.")}><Icon name="contacts" size={20}/></button><span/><button title="Labels" onClick={() => openView("labels")}><Icon name="plus" size={20}/></button></aside>
    </div>
    {mailboxMenu && <div className="mg-account-popover"><strong>{activeMailbox?.display_name || "mailG"}</strong><small>{activeMailbox?.address}</small>{mailboxes.map(mailbox => <button key={mailbox.id} onClick={() => { setMailboxId(mailbox.id); setMailboxMenu(false); setThreadId(null); }}><span className="mg-small-avatar">{mailbox.address[0].toUpperCase()}</span>{mailbox.address}</button>)}{connected ? <button onClick={() => void mailClient.signOut()}>Sign out</button> : <button onClick={() => mailClient.signIn()}>Connect Banger</button>}</div>}
    ...
    {chooserOpen && <div className="mg-modal-shade"><div className="mg-modal mg-mailbox-chooser"><h2>Choose your mailboxes</h2><p>Select the mailboxes you want to switch between in mailG.</p><div className="mg-chooser-list">{mailboxes.map(mailbox => <label key={mailbox.id}><input type="checkbox" checked={chooserIds.includes(mailbox.id)} onChange={event => setChooserIds(ids => event.target.checked ? [...ids, mailbox.id] : ids.filter(id => id !== mailbox.id))}/><span className="mg-small-avatar">{mailbox.address[0].toUpperCase()}</span><span><strong>{mailbox.display_name || mailbox.address}</strong><small>{mailbox.address}</small></span></label>)}</div><div className="mg-modal-actions"><span/>{selectedMailboxIds.length > 0 && <button onClick={() => setChooserOpen(false)}>Cancel</button>}<button className="mg-primary-button" disabled={!chooserIds.length} onClick={() => { if (!chooserIds.length) return; setSelectedMailboxIds(chooserIds); localStorage.setItem(`mailg-mailboxes:${workspaceId || "demo"}`, JSON.stringify(chooserIds)); if (!chooserIds.includes(mailboxId)) setMailboxId(chooserIds[0]); setChooserOpen(false); }}>Save selection</button></div></div></div>}
