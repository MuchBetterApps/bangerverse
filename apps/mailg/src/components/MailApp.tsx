...
export default function MailApp() {
  ...
  return <div className={`mg-app theme-${theme}`}>
    <header className="mg-topbar">
      ...
      <div className="mg-top-actions"><IconButton icon="help" label="Help" onClick={() => setToast("mailG is an open-source mail interface powered by Banger.")} /><IconButton icon="settings" label="Settings" onClick={() => setSettingsOpen(true)} /><IconButton icon="apps" label="Apps" onClick={() => setToast("More apps are coming soon.")} /><button className="mg-avatar" title={connected ? "Banger account" : "Connect Banger"} aria-label={connected ? "Banger account" : "Connect Banger"} aria-expanded={mailboxMenu} aria-controls="mailg-account-menu" onClick={() => setMailboxMenu(v => !v)}><Icon name="person" size={21}/></button></div>
    </header>
