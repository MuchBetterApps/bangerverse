...
Pulse does not store email bodies on disk. Its recent alert list holds only a short subject/snippet and any detected code in memory, capped at 20 items. Codes disappear from the alert list and tray ten minutes after the email arrived; expired codes cannot be copied. The settings window closes completely when you close it; the Rust tray process keeps the websocket connected. On macOS and supported Linux desktops, the latest fresh code appears beside the tray icon. The tray menu also shows it and has a one-click Copy action.
